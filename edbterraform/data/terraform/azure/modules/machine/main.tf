...
resource "azurerm_linux_virtual_machine" "main" {
  ...
  size = var.machine.spec.instance_type
  ...
  os_disk {
    caching = var.caching
    storage_account_type = var.machine.spec.volume.type
    disk_size_gb = var.machine.spec.volume.size_gb
  }
  ...
  # az vm image list --all --publisher="Rocky"
...
resource "null_resource" "setup_volume" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "/tmp/setup_volume.sh ${element(local.linux_device_names, tonumber(each.key))} ${each.value.mount_point} ${length(lookup(var.machine.spec, "additional_volumes", [])) + 1}  >> /tmp/mount.log 2>&1"
    ]
